...
/*FIXME: use simulator to check it instead of register load*/
...
/* FIXME: undefine SysTick, just use linux timer*/
#if defined(SysTick)
...
/* FIXME: just use linux timer
 *
 * val = ticks * configTICK_RATE_HZ
 * should test overflow
 * os tick timer is more accurate than ticks, that relationship is like second and minute
 */
...
typedef struct __xSTATIC_LIST
{
	UBaseType_t d1;
	void *d2;
	struct __xSTATIC_MINI_LIST_ITEM d3;
}__StaticList_t;
typedef struct __xSTATIC_QUEUE
{
   void *pvDummy1[ 3 ];
...
#define StaticQueue_t __StaticQueue_t
#endif
